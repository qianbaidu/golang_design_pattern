...
//  源地址： https://github.com/sevenelevenlee/go-patterns/blob/master/14-state-pattern/state.go
/*
	入口类包含子类的接口引用
...
func main() {
	func() {
		...
		if context.HealthValue != 15 {
			panic("health value err, expected is 15")
		}
		if _, ok := context.State.(*NormalState); !ok {
			panic("state  err, expected is normal state")
		}
	}()
	...
	func() {
		...
		if context.HealthValue != 9 {
			panic("health value err, expected is 9")
		}
